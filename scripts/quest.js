    ...
    for (var i = 0; i < questIds.length; i++) {
        ...
        try {
            ...
            function processQuestCheckData(questCheckData) {
                ...
                // Print npcValue for debugging
                var npcValue = Packages.provider.DataTool.getInt("npc", questCheckData, 0);
                if (DEBUG_MODE) print("npcValue for questId " + questId + ": " + npcValue);
                // Check for NPC requirements in Check.img
                var shouldAddNPC = false;
                // Check if this is a talk-only quest (no items or mobs required)
                var hasItems = questCheckData && questCheckData.getChildByPath("item");
                var hasMobs = questCheckData && questCheckData.getChildByPath("mob");
                if (!hasItems && !hasMobs) {
                    // Talk-only quest, always add the NPC
                    shouldAddNPC = true;
                } else {
                    // Check if we have completed the other requirements
                    ...
                    // Only add NPC if all other requirements are completed
                    if (itemsCompleted && mobsCompleted) {
                        shouldAddNPC = true;
                    }
                }
                if (npcValue > 0 && shouldAddNPC) {
                    ...
                    if (DEBUG_MODE) {
                        print("Quest " + questId + ": Adding talk NPC " + npcName + " (ID: " + npcValue + "), location: " + location + ", mapId: " + mapId);
                    }
            ...
            // Process both /0 and /1 for check.img.xml
